...
| `meshgen.hml` | The procedural mesh DSL and every world mesh: tree LOD0/1/2, bush, rock, crate, barrel, lantern post, contact blob, enemy ×6 × 3 LODs (rigid parts), **TWO villager classes × 3 LODs** (SETTLEMENT §7.4: named 210/90/28 switching at 12/28 m, ambient 48/28/12 at 10/22 m — six meshes out of four shapes, so the two classes share one silhouette grammar and differ only in detail; per-character identity is `meshgen_npc_scale`/`meshgen_npc_tint`, a uniform scale and a [200,255] tint applied through `mesh_emit_yaw`, at ZERO triangles), the LANTERN viewmodel (the only one the renderer is bound to) and five retained gun viewmodels — **42 meshes, 2982 triangles, built in ~33 ms** (the villager gained LOD1/LOD2 and the four humanoid enemies gained a violet chest panel at every tier; the six viewmodel slots moved 28..33 → 30..35, symbolically, nothing names them by number). Bakes vertex colour + creased AO at build time, then applies a soft luminance floor of 72 (`MG_LUM_FLOOR`) so no vertex in the game is darker than the atlas minimum. UVs are bound per MATERIAL, not per atlas cell: `assets.hml` calls `meshgen_set_material_uv(MG_MAT_*, u0,v0,u1,v1)` with texgen's `CELL_*` rects **before** `meshgen_build_all()`. | `mg_box, mg_box_ex, mg_prism, mg_taper, mg_fan_disc, mg_lathe, mg_spike, mg_panel, mg_ground_quad, mg_set_jitter, meshgen_build_all, meshgen_bake_all, meshgen_set_material_uv, meshgen_set_sun_azimuth, meshgen_mesh, meshgen_name, meshgen_tris, meshgen_budget, meshgen_kind, meshgen_radius, meshgen_view_yaw, meshgen_count, meshgen_enemy_mesh, mg_head_shoulder, mg_head_width, mg_shoulder_width, mg_protrusion, mg_protrusion_axis, MESH_TREE0/1/2, MESH_BUSH, MESH_ROCK, MESH_CRATE, MESH_BARREL, MESH_LANTERN, MESH_BLOB, MESH_WISP0..MESH_BULWARK2 (class ×3 LODs), meshgen_npc_mesh, meshgen_villager_mesh, meshgen_villager_lod1_m, meshgen_villager_lod2_m, meshgen_npc_scale, meshgen_npc_tint, MESH_NPC0 (= MESH_NPC), MESH_NPC1, MESH_NPC2, MESH_VLG0, MESH_VLG1, MESH_VLG2, MG_NPC_LOD1_M, MG_NPC_LOD2_M, MG_VLG_LOD1_M, MG_VLG_LOD2_M, MG_VLG_NAMED, MG_VLG_AMBIENT, MG_VLG_CLASSES, MG_NPC_MABEL..MG_NPC_PIP, MG_NPC_NAMED, MG_TINT_FLOOR, MESH_LANTERN_VM (= MESH_SPARROW_VM, the alias), MESH_TINKER_VM..MESH_EMBERLANCE_VM, MESH_COUNT, ENEMY_WISP..ENEMY_BULWARK, MG_MAT_*, MG_KIND_*, MG_LUM_FLOOR, MG_VM_LUM_FLOOR, meshgen_vm_bob` |
...
| `hubgen.hml` | **RE-SCOPED by the settlement-render task (SETTLEMENT.md §8.2 / §11.2): the ARCHETYPE MESH LIBRARY and the warm dome; LAYOUT lives in `src/sim/settle.hml` because it is world state, must run headless, and `src/sim/**` may not import `src/art/**`.** 8 archetypes x 4 construction states x 9 variants (1 B0 + 4 B1 sectors + 4 B2 sectors) = **232 meshes, 5162 triangles, built in 49.6 ms**. Reproduces SETTLEMENT.md §7.1's authored/drawn table exactly. Also owns THE WARM DOME (§6.4): the settlement's glow written into skygen's horizon band, **zero triangles**, riding the existing rolling re-bake via `hubgen_sky_slice` (a drop-in replacement for `skygen_rebake_slice`). | `hubgen_bind_atlas, hubgen_build_all, hubgen_is_built, hubgen_total_tris, hubgen_mesh_count, hubgen_mesh, hubgen_tris, hubgen_radius, hubgen_mesh_table, hubgen_tris_table, hubgen_radius_table, hubgen_aabb_table, hubgen_aabb, hubgen_bake_all, hubgen_arch_authored, hubgen_arch_drawn, hubgen_eaves, hubgen_footprint_x/z, hubgen_sector_faces, hubgen_sector_of, hubgen_variant, hubgen_render_yaw, hubgen_window_count, hubgen_window_local, hubgen_dome_set, hubgen_dome_clear, hubgen_dome_active, hubgen_dome_u, hubgen_dome_half_angle, hubgen_dome_lift, hubgen_dome_paint, hubgen_dome_paint_all, hubgen_dome_texels, hubgen_sky_slice, HG_B_*, HG_CON_*, HG_RUNG_*, HG_SECTORS, HG_VARIANTS, HG_SLOTS, HG_LOD_B1_M, HG_LOD_B2_M, HG_EAVE_M, HG_FOOT_M, HG_DOME_LUX_FULL` |
...
| `affinity.hml` | **The KINETIC / MAGIC axis** (MAGIC_KINETIC §1.1, §2, §3, §5.3). A pure table module and its three laws; no state, no allocation, no world, no RNG, no clock. A **LEAF of the sim** — it imports `src/core/config.hml` and nothing else, because `combat.hml` imports it and `spells.hml` imports `combat.hml`. Eight class rows (the six of GAME_DESIGN §2.2, **extended not replaced**, plus forward rows for `AFF_CLS_GLOAM = 6` / `AFF_CLS_SNUFFER = 7` that `config.hml` must adopt as `g_ENEMY_GLOAM` / `g_ENEMY_SNUFFER` when `g_ENEMY_KIND_COUNT` goes 6 → 8; `aff_config_agrees()` asserts the table is never narrower than config). **LAW 1** the kinetic column is exactly 1.00 on all six shipped classes, pinned with six literals, so `x * 1.0` is IEEE-exact and the measured GDD §2.4 TTK tables cannot move. **LAW 2** no multiplier below `AFF_FLOOR_MULT = 0.15` and none zero, so the worst wrong-hand PENALTY in the game is 6.67× and never a wall. **LAW 3** every multiplier outside the felt band [0.80, 1.25] must carry a cue at 12 px and at 6 px on a channel that survives, and `AFF_CH_NUMBER` is declared illegal as a primary cue so an unperceivable resistance is a build failure. | `aff_dtype_of_kind, aff_class_of_model, aff_is_bodied, aff_base_mult, aff_body_mult, aff_pool_mode, aff_pool_mult, aff_pool_hp, aff_windup_mult, aff_resisted, aff_vulnerable, aff_penalty, aff_ttk_ratio, aff_best_hand, aff_cue_of, aff_cue_channel_of, aff_cue_range_m, aff_cue_built, aff_ext_of, aff_ext_channel_of, aff_ext_range_m, aff_ext_built, aff_cue_is_built, aff_cue_built_ok, aff_cue_built_count, aff_cue_name, aff_channel_name, aff_class_name, aff_width_ok, aff_kinetic_anchor_ok, aff_solo_ok, aff_cue_ok, aff_dtype_map_ok, aff_config_agrees, aff_kind_ids_ok, aff_selfcheck, AFF_CLS_*, AFF_DMG_*, AFF_POOL_*, AFF_CUE_*, AFF_CH_*, AFF_BUILT_*, AFFCHK_*, AFF_FLOOR_MULT, AFF_MAX_TTK_RATIO, AFF_GOOD_HAND_MULT, AFF_TELL_LO, AFF_TELL_HI, AFF_WINDUP_MAGIC, AFF_SPITTER_SAC, AFF_WARDEN_PLATE_MAGIC, AFF_SNUFFER_*` |
| `combat.hml` | Hitscan: DDA ray vs heightfield + ray vs entity AABB, headshot volume, damage application, hit events. Authoritative. **Applies the affinity axis** — one multiply in `combat_apply_damage`, plus pool routing for the Warden plate and the Snuffer shell. The affinity verdict of the last call is scratch on the context (never hashed, never on the wire) and **every caller other than `combat_fire` must read it through the four accessors** to set its own marker bits and to show a non-zero damage number when the damage all went into a pool. | `combat_raycast, combat_apply_damage, combat_fire`, plus `combat_last_resisted, combat_last_vulnerable, combat_last_pool_dmg, combat_last_dtype, combat_affinity_ok, combat_hm_bits_ok, HM_MAGIC (16), HM_RESISTED (32), SFX_HIT_THOCK (9), HIT_FLASH_RESISTED_CS (3), STAT_RESISTED_HITS, STAT_POOL_DAMAGE` |
...
| `progression.hml` | XP curve `500+250L+25L²`, level, unlock ladder, Wickmarks, Ember Streaks (Flare/Drone/Dawnfall). **§7b (stats task) adds the STAT-POINT BUDGET**: `prog_points_earned(st)` = `1×(level−1) + 1×wickmarks`, derived — no new state slot, no `PROG_VERSION` bump. It is **not** `PROG_STAT_POINTS`, which stays the Wickmark-only balance guarded by `PROGVAL_STATS`. | `prog_add_xp, prog_level, prog_unlocked, prog_streak_step, UNLOCK_*, prog_points_earned, prog_points_earned_at, PROG_PTS_PER_LEVEL, PROG_PTS_PER_WICKMARK, PROG_PTS_CAP, PROGCHK_POINTS` |
| `stats.hml` | The five stats (Handling/Capacity/Lantern/Vitality/Attunement, 0..20, neutral 6), five 3-node/3-rank branches, and every **effectiveness** curve. DIRECTION §7 §9. **Gates effectiveness, NEVER access** — no exported function returns "may I", every multiplier is clamped off zero, and `stats_selfcheck`'s `SKCHK_TRAP` sweeps the whole input space to enforce it. Weapon penalties scale by DEMAND (melee 0.00 / primitive 0.45 / manufactured 1.00), so a low stat never punishes melee or the opening hour. Carry capacity is equipment; overload slows, never refuses. Flat `array` of `SKS_FIELDS` (22) i32 = 88 B, imports `prog_points_earned` one-way (progression must never import this). | `SK_{HANDLING,CAPACITY,LANTERN,VITALITY,ATTUNEMENT,COUNT,STAT_MIN,STAT_MAX,NEUTRAL}`, `SK_DEMAND_{MELEE,PRIMITIVE,MANUFACTURED,COUNT}`, `SK_N_*` (15 nodes), `SK_{NODE_COUNT,NODES_PER_STAT,NODE_RANK_MAX,BRANCH_PER,MULT_FLOOR,MULT_CEIL,SLOTS_BASE,SLOT_PER}`, `SK_EQ_{NONE,BANDOLIER,POUCHES,PACK,ALL}`, `SK_VERSION, SKS_{VER,STAT,NODE,RESPECS,FIELDS}`, `stats_{new,reset,copy,get,rank,respecs,spent,unspent,unspent_of,can_raise,can_raise_of,raise,raise_of,branch_points,branch_spent,node_open,can_rank,rank_up,respec}`, `stats_{stat_name,node_name,node_stat,node_req,node_of,demand_of_weapon}`, `stats_{recoil,sway,ads,reload}_mult[_at|_wpn]`, `stats_{equip_kg,carry_kg,overload01,move_mult,ammo_mult,material_cap}[_at]`, `stats_{light_radius_mult,channel_mult,channel_s,upkeep_mult,wickline_mult_at,wickline_m}`, `stats_{hp_mult_at,hp_max,revive_hp,stagger_resist}[_at]`, `stats_{spell_slots,potency_mult,cooldown_mult}[_at]`, `stats_{serialize,deserialize,hash,effect_hash,validate,validate_of,selfcheck}`, `SKVAL_*`, `SKCHK_*`, `stats_ui_{col_px,rows,name_px}` |
...
| `npc.hml` | **The villagers — DIRECTION §6.** Five named (Mabel/Odo/Wick at Village, Connie/Pip at Town) plus a 0/4/9/18 ambient crowd, all present-or-absent as a pure function of `settle_tier_ever`. Owns: the **OFFER TABLE** (21 rows of `who, kind, arg, tier, regard`) which is the crafting and trading interface — `npc_weapon_source(store, st, NPC_VW_BOLT)` answers *"who will make me a bolt-action"* and `npc_weapon_gate` answers *why not*; **REGARD** 0..255, saturating and **monotone by construction** (`npc_add` clamps a negative delta away — the grid decays, the people do not); the **TOPIC** ladder, whose third rung is `npc_dark_scan`'s eight-octant light sample turned into *"the east road has gone dark"* — the whole of the entropy UI, with no meter anywhere; stations and a four-line schedule; and the **VISIBLE SET**, which applies SETTLEMENT §7.4/§7.6's caps (8 visible, 1 named at LOD0) plus a 300-drawn-triangle cap enforced by **demoting the farthest villager, never dropping one**. Imports `mathx`, `daycycle`, `settle`, `weapons`. No art, no clock, no RNG state: every position, topic and offer is f(seed, settlement, tick). Ambient villagers are **not saved** — 5 rows × 4 i32 is the entire social state of a City. | `NPC_{MABEL,ODO,WICK,CONNIE,PIP,NAMED,NONE}`, `NPC_CLASS_{NAMED,AMBIENT,COUNT}`, `NPC_AMB_{CAMP,VILLAGE,TOWN,CITY}`, `NPC_MAX`, `NPC_{REGARD_MAX,BAND_*,BAND_T_*,CR_*,CRK_*}`, `NPC_T_*`, `NPC_LINES`, `NPC_DIR_*`, `NPC_DARK_{R_M,LUX,RATIO,EVERY}`, `NPC_OFF_*`, `NPC_VW_{BOLT,REVOLVER,PUMP,PIPESMG,COUNT}`, `NPC_EV_*`, `NPC_TRIS_{NAMED,AMB}_L0..2`, `NPC_LOD{1,2}_{NAMED,AMB}_M`, `NPC_{VIS_MAX,LOD0_MAX,TRIS_BUDGET,VIS_*,VIS_STRIDE}`, `NPC_{SAVE_COLS,META_COLS,VERSION,TALK_M,RADIUS_M}`, `npc_{valid,arrive_tier,named_at_tier,ambient_at_tier,pop_at_tier}`, `npc_{new,refresh,step}`, `npc_{present,named_count,ambient_count,count,slot_class,slot_who,slot_of,arrived_day,last_arrival}`, `npc_offer_{rows,who,kind,arg,tier,regard,first,count,open,open_count}`, `npc_offers_open`, `npc_weapon_{source,gate}`, `npc_{pos_fill,x,y,z,yaw,nearest,talk_target}`, `npc_dir_{bearing,of_bearing}`, `npc_dark_{scan,dir,lux,ring,reportable}`, `npc_{band_of,regard,band,met,talks,last_talk_day,talk,has_new_line,credit}`, `npc_topic{,_fill,_arg}`, `npc_{tris_of,visible_fill,vis_tris}`, `npc_{save_rows,save_fill,meta_fill,load,hash,selfcheck}`, `NPCCHK_*` |
...
| `settle_render.hml` | **NEW (settlement-render).** The settlement emit walk: the green's terraced deck + kerb + roads as chunk meshes (one per road, so the frustum drops the ones behind you), every building as a three-rung LOD prop with its B1 sector, SETTLEMENT.md §7.6's caps enforced **in the walk** (demote one rung, never drop), the lit windows as FX cards, and the §7.3 fog ramp. **Never imports `src/art/**`** — the mesh library arrives through `settle_render_bind` as four flat tables, and the atlas cells and palette colours through `settle_render_bind_cells` / `_bind_ground_rgb` / `_bind_glow`. Measured: emit walk **0.037 ms/frame**, ground build **4.76 ms** cached on (seed, tier). | `settle_render_bind, settle_render_is_bound, settle_render_bind_atlas, settle_render_bind_glow, settle_render_bind_cells, settle_render_bind_ground_rgb, settle_render_sector, settle_render_yaw, settle_render_ground_build, settle_render_ground_dirty, settle_render_ground_count, settle_render_ground_mesh, settle_render_ground_tris, settle_render_ground_bake, settle_render_ground_push, settle_render_emit, settle_render_stats, settle_render_fog_far, settle_render_dome_dx/dz/lux, SR_MAX_BLD_FULL, SR_MAX_BLD_BLOCK, SR_MAX_GLOW, SR_PLAYER_BUILD_TRIS, SR_LOD_B1_M, SR_LOD_B2_M, SR_STAT_*, g_SR_FOG_FAR_SETTLEMENT, g_SR_FOG_RAMP_M` |
| `world_render.hml` | The frame-graph orchestrator: owns the four batches, the RenderEnv, the view/MVP/frustum, the sky mapping and the render-world registry; runs **stages 8–23 (`frame_render`) and 25–28 (`frame_present`)** of §4 in order. **The only file that knows the layer order.** Stage 6 (`tod_eval`), stage 7 (skygen) and stage 24 (the HUD) are the CALLER's — see 5.6b. | `frame_init, frame_batches, frame_begin, frame_render, frame_present, frame_bind_textures, frame_bind_fx_atlas, frame_bind_assets, frame_set_camera, frame_set_clear, frame_set_sheet_clear, frame_set_options, frame_set_capture, frame_capture, frame_set_hud_solid_uv, frame_set_vignette_colour, frame_set_flash_colour, frame_env, frame_mvp, frame_frustum, frame_batch_sky/world/fx/hud, frame_camera_x/y/z, world_clear, world_chunk_add, world_prop_add, world_fx_add, world_chunk_count, world_chunk_mesh, world_prop_count, world_prop_tag/x/y/z/scale, world_fx_count, terrain_set, terrain_seed, terrain_ridge, terrain_h, vnoise2, h2, sky_basis, sky_uv_at, sky_u, sky_v, g_D2R, g_NEAR, g_GUARD, g_CAP_*, g_FXK_SPRITE/RAIN/TRACER, g_LOD_FOG_CARD` |
...
//  --town <0..3> --at-town --demo-ui            (DEV; see 5.7c)
```

#### 5.7c W6 — the ten systems, wired  *(added by the `wire-it-up` task)*

Ten modules (`melee`, `spells`, `stats`, `settle`, `npc`, `affinity`, `hubgen`,
`settle_render`, spell FX) were built by other tasks and called by nobody. `sim.hml` predates all
ten and is not the wiring task's file, so **the authority calls them**, in `server_tick` between
`sim_step` and `sv_script`, in this order — and the order is API for the same reason §4's is:

```
1. melee_tick        writes damage into the world
2. spell_cast/step   writes damage into the world
3. the modal UI      reads the world, writes the player's sheet
4. settle_step       reads the lit lanterns, writes LUX and buildings
5. npc_step          reads the settlement, writes only itself
```

**Input stays inside the nine integers.** CLAUDE.md §7 admits no tenth field, so the four unused
button bits carry the new verbs and the CURSOR they move lives on the authority (it must rewind,
save and replay like everything else — the same argument `melee.hml` makes for its press edge):

```
BTN_MELEE(128)    swing            BTN_GRENADE(256)  cast / confirm a panel row
BTN_SWITCH(512)   next spell / row BTN_INV(2048)     character sheet
BTN_BUILD(1024)   trade panel      BTN_USE(64)       lantern if in reach, else talk
```

```hemlock
// ---- src/game/server.hml — added surface -----------------------------------
server_settle(sv): object      server_npcs(sv): object     server_melee(sv): object
server_spells(sv): array       server_stats(sv): array
server_trades/trades_refused/points_spent/talks/ui_panel/lan_count(sv): i32
server_settle_tier(sv): i32    server_w6_hash(sv): i32
server_settle_boot_tier(sv, tier: i32): i32     // DEV, drives settle_load
server_teleport(sv, x: f64, z: f64): i32        // DEV
constants UI_PANEL_{NONE,CHAR,TRADE}, UI_MSG_{NONE,POINT,NO_POINTS,STAT_MAX,
          TRADED,NO_TRADE,UNBUILT,NOBODY}, SV_MELEE_SLOTS(2), SV_LAN_CAP(96),
          SV_TRADE_AMMO(24), SV_TRADE_MFG_RESERVE(120), SV_TALK_R_M

// ---- src/game/client.hml — added surface -----------------------------------
client_mel_wpn/mel_phase/mel_swings/mel_hits(cl): i32
client_spell_id/spell_slots/spell_have/cloaked/in_light/spell_casts/spell_result(cl): i32
client_oil01/oil/oil_tank/spell_cd01/spell_chan01/cast_t(cl): f64
client_stat(cl, i: i32): i32   client_points/points_spent(cl): i32
client_settle_{name,tier,tier_ever,core,core_need,pts,pts_need,buildings,
               brownout,inside,lux}(cl): i32
client_settle_{x,z,dist,bearing}(cl): f64      client_promote_t/arrive_t(cl): f64
client_npc_count/npc_named/talk_who/talk_topic/talk_arg/talk_band/
       talk_offers/talk_new/talk_prompt(cl): i32          client_talk_dist(cl): f64
client_ui_panel/ui_row/ui_rows/ui_msg(cl): i32
// presentation — the client owns every string; no sim module has one
client_npc_name/npc_place(who: i32): string    client_spell_name(id: i32): string
client_melee_name(m: i32): string              client_stat_short/stat_name(i: i32): string
client_settle_tier_name(t: i32): string        client_site_name(n: i32): string
client_offer_label(kind: i32, arg: i32): string
client_ui_msg_text(msg: i32): string
client_talk_line(who: i32, topic: i32, arg: i32): string
```

**The lantern feed is the whole loop, in one function.** `server.hml`'s `sv_lan_gather` walks the
world once a second, collects every `KIND_LANTERN` whose model is `SIM_LANTERN_LIT` inside the
settlement ring, and hands them to `settle_lanterns_set` as `(x, z, tier, wick)`. **`wick` is a
MAGNITUDE, not a flag: full is `SET_WICK_FULL` = 256**, against `SET_CORE_VILLAGE` = 512 and
`SET_LUX_PER_POINT` = 256. Rows `[0, lan_civic)` belong to the settlement (its Great Lantern and
its own street lamps) and survive the sweep; rows `[lan_civic, lan_n)` are the world's posts and are
rebuilt from it.

**Two gaps are open and are NOT faked** — see the task report:
`NPC_OFF_WEAPON`'s four arguments (`NPC_VW_BOLT/REVOLVER/PUMP/PIPESMG`) have **no rows in
`weapons.hml`**, so taking one returns `UI_MSG_UNBUILT` and grants nothing; and `save.hml` does not
persist any W6 store, so a save round-trips the sim but not the settlement, the roster or the sheet.
